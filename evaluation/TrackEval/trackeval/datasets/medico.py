...
class Medico(_BaseDataset):
    ...
    def get_default_dataset_config():
        ...
        default_config = {
            'GT_FOLDER': os.path.join(code_path, 'data/gt/medico/'),  # Location of GT data
            'TRACKERS_FOLDER': os.path.join(code_path, 'data/trackers/medico/'),  # Trackers location
            'OUTPUT_FOLDER': "output",  # Where to save eval results (if None, same as TRACKERS_FOLDER)
            'TRACKERS_TO_EVAL': None,  # Filenames of trackers to eval (if None, all in folder)
            'CLASSES_TO_EVAL': ['sperm', 'small-or-pinhead', 'cluster'],  # Valid: ['sperm', 'small-or-pinhead', 'cluster']
            'BENCHMARK': 'Medico',  # Valid: 'Medico'
            'CONVERT_FROM_YOLO': False,  # Whether boudning box coordinats use the yolo format or not
            'SPLIT_TO_EVAL': 'test-sperm',  # Valid: 'train', 'test', 'all'
    ...
    def __init__(self, config=None):
        ...
        self.convert_from_yolo = self.config["CONVERT_FROM_YOLO"]
        ...
        # Get classes to eval
        self.valid_classes = ['sperm', 'small-or-pinhead', 'cluster']
        self.class_list = [cls.lower() if cls.lower() in self.valid_classes else None
                           for cls in self.config['CLASSES_TO_EVAL']]
        if not all(self.class_list):
            raise TrackEvalException('Attempted to evaluate an invalid class.')
        self.class_name_to_class_id = { 'sperm': 0, 'small-or-pinhead': 1, 'cluster': 2 }
        self.valid_class_numbers = list(self.class_name_to_class_id.values())

        # Get sequences to eval and check gt files exist
        self.seq_list, self.seq_lengths, self.ini_data = self._get_seq_info()
        ...
        if self.config["SEQ_INFO"]:
            ...
            with open(seqmap_file) as fp:
                ...
                for i, row in enumerate(reader):
                    ...
                    ini_data.read(ini_file)
                    seq_lengths[seq] = int(ini_data['Sequence']['seqLength'])
        return seq_list, seq_lengths, ini_data["Sequence"]

    def _convert_from_yolo(self, det):      
        float_x_center = float(self.ini_data["imWidth"]) * det[0]
        float_y_center = float(self.ini_data["imHeight"]) * det[1]
        float_width = float(self.ini_data["imWidth"]) * det[2]
        float_height = float(self.ini_data["imHeight"]) * det[3]
        det[0] = int(float_x_center - float_width / 2)
        det[1] = int(float_y_center - float_height / 2)
        det[2] = int(float_width)
        det[3] = int(float_height)
        return det
                    ...
                    if is_gt:
                        ...
                        raise TrackEvalException(
                            'Cannot convert tracking data from tracker %s, sequence %s to float. Is data corrupted?' % (
                                tracker, seq))
                try:
                    if self.convert_from_yolo:
                        dets = np.apply_along_axis(self._convert_from_yolo, 1, time_data[:, 2:6])
                    else:
                        dets = np.atleast_2d(time_data[:, 2:6])
                    raw_data['dets'][t] = dets
